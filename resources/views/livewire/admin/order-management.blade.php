<div x-data="{
    ...
    <div x-show="activeTab === 'pending'" x-transition>
        @if($pending->isEmpty())
            @include('livewire.admin.partials.empty-state', ['icon' => 'bx-time-five', 'label' => 'No pending orders'])
        @else
            <div class="row g-3">
    ...
    <div x-show="activeTab === 'confirmed'" x-transition>
        @if($confirmed->isEmpty())
            @include('livewire.admin.partials.empty-state', ['icon' => 'bx-check-circle', 'label' => 'No confirmed orders'])
        @else
            <div class="row g-3">
    ...
    <div x-show="activeTab === 'served'" x-transition>
        @if($served->isEmpty())
            @include('livewire.admin.partials.empty-state', ['icon' => 'bx-dish', 'label' => 'No served orders awaiting payment'])
        @else
            <div class="row g-3">
    ...
    <div x-show="activeTab === 'paid'" x-transition>
        @if($paidOrders->isEmpty())
            @include('livewire.admin.partials.empty-state', ['icon' => 'bx-receipt', 'label' => 'No paid orders yet'])
        @else
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @foreach($paidOrders as $order)
                                <tr>
                                    <td><span class="fw-semibold text-muted small">{{ strtoupper(substr($order->uuid, 0, 8)) }}</span></td>
                                    <td>
                                    ...
                                    <td><span class="text-muted small">{{ $order->paid_at?->format('d M, g:i A') }}</span></td>
                                    <td>
                                        <div class="d-flex gap-2">
                                            ...
                                            @if($order->session && $order->session->status === 'active')
                                                <button type="button" class="btn btn-sm btn-outline-danger"
                                                    ...
                                                    <span wire:loading.remove wire:target="closeTableSession({{ $order->session->id }})">
                                                        <i class="bx bx-power-off me-1"></i> Close Session
                                                    </span>
                                                    <span wire:loading wire:target="closeTableSession({{ $order->session->id }})">
    ...
    <div x-show="showDetail"
        ...
        <div x-show="showDetail"
             ...
             class="order-detail-modal card shadow-lg">
                                    ...
                                    <template x-for="item in orderDetail.items" :key="item.name + item.qty">
                                        <tr>
                                            <td>
                                                ...
                                                <div class="text-muted" style="font-size:0.75rem;" x-text="item.note ? '📝 ' + item.note : ''" x-show="item.note"></div>
                                            </td>
                                            <td class="text-center" x-text="item.qty"></td>
                                            <td class="text-end small" x-text="'Rs. ' + parseFloat(item.price).toFixed(2)"></td>
                                            <td class="text-end small fw-semibold" x-text="'Rs. ' + parseFloat(item.subtotal).toFixed(2)"></td>
                                        </tr>
                                    </template>
                                </tbody>
                                <tfoot>
                                    <tr class="table-light">
                                        ...
                                        <td class="text-end fw-bold text-primary" x-text="'Rs. ' + parseFloat(orderDetail.total).toFixed(2)"></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
