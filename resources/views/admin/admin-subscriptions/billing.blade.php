...
<div>
            ...
            <div class="card-body p-4 p-xl-5 position-relative">
                <div class="row align-items-center">
                    <div class="col-md-9">
                        ...
                        <p class="mb-0 opacity-75">You do not need any subscription plan to interact with the system. All platform features are unlocked for you by default.</p>
                    </div>
                </div>
                <i class="bx bx-shield-quarter" style="font-size: 120px; opacity: 0.15; position: absolute; right: -10px; bottom: -20px;"></i>
            </div>
        </div>
    ...
    @else
        ...
        <div class="card border-0 shadow-sm bg-primary text-white mb-4 overflow-hidden">
            <div class="card-body p-4 p-xl-5 position-relative">
                        ...
                        @if($currentPlan && $currentPlan->id !== 0)
                            <div class="d-flex flex-wrap gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <div class="fw-semibold">{{ $currentPlan->duration_value }} {{ Str::plural($currentPlan->duration_unit, $currentPlan->duration_value) }}</div>
                                </div>
                                <div>
                                    <p class="opacity-75 mb-0" style="font-size:0.72rem;">PRICE</p>
                                    <div class="fw-semibold">{{ $currentPlan->currency }} {{ number_format($currentPlan->price, 2) }}</div>
                                </div>
                            </div>
                            ...
                            @if($sub?->expires_at)
                                ...
                                @endphp
                                <div class="progress rounded-pill mb-2" style="height:6px; background:rgba(255,255,255,0.25);">
                        ...
                        @else
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                @foreach(['Generate Menu QR Code', 'Add Menu Images or PDF', 'Scan to View Menu'] as $f)
                                    <span class="badge rounded-pill fw-normal px-3 py-2" style="background:rgba(255,255,255,0.2);">
                ...
                <i class="bx bx-receipt" style="font-size: 120px; opacity: 0.1; position: absolute; right: -10px; bottom: -20px;"></i>
            </div>
        </div>
    @endif
    ...
    <div class="mb-4">
        ...
        <div class="row g-4">
            ...
            {{-- Free Plan (Static) --}}
            <div class="col-md-4">
                <div class="card border-0 shadow-sm h-100 @if(!$admin->is_super_admin && (!$currentPlan || $currentPlan->id === 0)) border-primary @endif">
                    @if(!$admin->is_super_admin && (!$currentPlan || $currentPlan->id === 0))
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                <h5 class="fw-bold mb-0">Free Plan</h5>
                                <p class="text-muted small mb-0">Always free, forever</p>
                            </div>
                            <div class="rounded-circle d-flex align-items-center justify-content-center"
                                 style="width:40px;height:40px;background:#f0f4ff;">
                                <i class="bx bx-user text-primary"></i>
                            </div>
                        </div>
                        <div class="display-6 fw-bold mb-3">NPR 0 <span class="fs-6 text-muted fw-normal">/ forever</span></div>
                        <hr class="my-3">
                        <ul class="list-unstyled mb-0">
                            @foreach(['Generate Menu QR Code', 'Add Menu Images or PDF', 'Scan to View Menu'] as $feature)
                                <li class="d-flex align-items-center gap-2 mb-2 small">
                                    <i class="bx bx-check text-success fs-5"></i>{{ $feature }}
                                </li>
                            @endforeach
                        </ul>
                    </div>
            ...
            @forelse($plans as $plan)
                ...
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm h-100 @if($isCurrentPlan) border border-success @endif"
                         style="@if($loop->first && !$isCurrentPlan) background: linear-gradient(135deg, #f8f9ff 0%, #eef2ff 100%); @endif">
                        @if($isCurrentPlan)
                        ...
                        <div class="card-body p-4">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <div>
                                    ...
                                    <p class="text-muted small mb-0">Premium Access</p>
                                </div>
                                <div class="rounded-circle d-flex align-items-center justify-content-center"
                            ...
                            <div class="display-6 fw-bold mb-3">
                                ...
                                <span class="fs-6 text-muted fw-normal">/ {{ $plan->duration_value }} {{ Str::plural($plan->duration_unit, $plan->duration_value) }}</span>
                            </div>
                            <hr class="my-3">
                                ...
                                <ul class="list-unstyled mb-0">
                                    @foreach($plan->features as $fKey => $fVal)
                                        <li class="d-flex align-items-center gap-2 mb-2 small">
                                            ...
                                            <span><strong>{{ Str::title(str_replace('_', ' ', $fKey)) }}</strong>{{ $fVal ? ': ' . $fVal : '' }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            @else
        ...
        <div class="card border-0 shadow-sm overflow-hidden">
            <div class="card-body p-0">
                <div class="px-4 py-3 border-bottom d-flex align-items-center justify-content-between">
                    ...
                    <span class="badge bg-secondary-subtle text-secondary rounded-pill">{{ $billingHistory->count() }} records</span>
                </div>
                                ...
                                <tr>
                                    <td class="px-4 py-3">
                                        ...
                                        <div class="text-muted small">
                                            {{ $record->plan?->duration_value }} {{ $record->plan ? Str::plural($record->plan->duration_unit, $record->plan->duration_value) : '' }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-3">
                                        ...
                                        @elseif($record->status === 'expired')
                                            <span class="badge bg-secondary-subtle text-secondary rounded-pill px-3">Expired</span>
                                        @else
                                            <span class="badge bg-warning-subtle text-warning rounded-pill px-3">{{ Str::title($record->status) }}</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
